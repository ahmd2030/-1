...
const AsyncImageGenerator = ({ generateData, base64Image }: { generateData: any; base64Image: string }) => {
    ...
    .then(d => {
       ...
       if (d.imageUrl) {
         ...
         try {
           ...
           toast.success(" „ «· Ê·Ìœ »‰Ã«Õ! «·’Ê—… „ «Õ… ›Ì „⁄—÷ «·’Ê— «·„Ê·œ….");
         } catch(e) {}
       }
    })
    .catch(e => {
  ...
  if (loading) return <p className="text-xs text-green-600 mt-2 font-medium">? Ã«—Ì  Ê·Ìœ «·’Ê—… ›Ì «·Œ·›Ì… (ﬁœ Ì” €—ﬁ 30 À«‰Ì…)...</p>;
  if (data.error) return <p className="text-xs text-red-600 mt-2">? Œÿ√: {data.error}</p>;
  return (
    <div className="flex flex-col gap-2 mt-4 bg-slate-50 p-3 rounded-xl border">
      <p className="text-xs text-green-600 font-medium">?  „  Ê·Ìœ «·’Ê—… »‰Ã«Õ!</p>
        ...
        <div className="bg-slate-900 text-white px-6 py-4 flex flex-row-reverse justify-between items-center z-10">
          <div className="flex items-center gap-3">
            <div className="text-right">
              <h2 className="font-bold text-lg">«·„Œ—Ã «·–ﬂÌ ï AI Director</h2>
              <p className="text-xs text-slate-400">„ Œ’’ ›Ì «·√“Ì«¡ Ê«·„Ê÷… ï Ì»ÕÀ ⁄·Ï «·≈‰ —‰  ï Ì –ﬂ— «·„Õ«œÀ…</p>
            </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6 bg-slate-50 relative">
          {messages.length === 0 && (
            <div className="h-full flex flex-col items-center justify-center text-center max-w-md mx-auto">
              ...
              <h3 className="text-2xl font-bold mb-2 text-slate-800">√Â·«° √‰« «·„Œ—Ã «·–ﬂÌ</h3>
              <p className="text-slate-500 max-w-sm text-sm leading-relaxed">
                «—›⁄ ’Ê— „‰ Ã« ﬂ Ê√Œ»—‰Ì ﬂÌ›  —Ìœ ≈Œ—«ÃÂ«. Ì„ﬂ‰‰Ì  Õ·Ì· «·’Ê—° «·»ÕÀ ⁄‰ √ÕœÀ ’ÌÕ«  «·„Ê÷…° Ê Ê·Ìœ ’Ê— «Õ —«›Ì….
              </p>
              <div className="mt-6 grid grid-cols-1 gap-2 w-full max-w-sm">
                {[
                  "„« ÂÌ √»—“ ’ÌÕ«  „Ê÷… «·√ÿ›«· ·„Ê”„ «·Œ—Ì› 2025ø",
                  "Õ·¯· Â–Â «·’Ê—… Ê«ﬁ —Õ ﬂÌ› √Õ”¯‰ «·” «Ì·",
                  "Ê·¯œ ’Ê—« ·Â–Â «·„‰ Ã«  „⁄ ‘⁄«— Baby Rose »«·Ê—œÌ",
                ].map((s) => (
                  <button
                    key={s}
                    onClick={() => setInput(s)}
      ...
      {showGallery && (
        <div className="w-80 bg-slate-50 border-r flex flex-col z-20">
          <div className="p-4 bg-slate-900 text-white flex justify-between items-center">
            <h3 className="font-bold text-sm">„⁄—÷ «·’Ê— «·„Ê·œ…</h3>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4">
            {galleryImages.length === 0 ? (
              <p className="text-xs text-slate-500 text-center mt-10">·«  ÊÃœ ’Ê— „Ê·œ… »⁄œ.</p>
            ) : (
                ...
                <div key={i} className="group relative rounded-xl overflow-hidden border shadow-sm">
                  ...
                  <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center gap-2">
                     ...
                     <button onClick={() => {
                        ...
                        setInput(prev => prev + " [«‰Ÿ— ·Â–Â «·’Ê—…: " + url + "] ");
                        toast.success(" „ ≈œ—«Ã —«»ÿ «·’Ê—… ›Ì „—»⁄ «·„Õ«œÀ…");
